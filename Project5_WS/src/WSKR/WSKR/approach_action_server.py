...
This is the brain of the approach behavior. A client (typically a GUI like
``Start_Aruco_Approach``) sends an ``ApproachObject`` action goal naming an
ArUco tag ID (or a toy bbox seeded by an external detector). This node then:
    ...
    2. Feeds the fused heading-to-target (from ``dead_reckoning_node``) plus
       the 11 whisker drive-distances into the trained MLP autopilot, which
       outputs a drive/rotate command.
    3. Publishes that command on ``WSKR/cmd_vel`` — the arduino serial bridge
       forwards it to the robot base.
    4. Succeeds when the tracked bbox's heading maps to a short whisker
       (impingement), aborts on timeout / loss / reacquisition failure.
...
Topics:
    ...
    publishes   WSKR/cmd_vel                     — autopilot Twist (m/s, rad/s).
...
from .model_runtime import JsonMLPModel, TemporalInputBuilder
...
class WSKRApproachActionServer(Node):
    def __init__(self) -> None:
        ...
        # Action-filter toggles. Match training defaults by applying both,
        # but disable either at runtime via SetParameters to test the raw
        # MLP output. Set apply_stiction_hysteresis=False to skip snap /
        # deadband / hysteresis; set apply_saturation=False to skip output
        # clamping (use with caution — removes the safety limit too).
        self.declare_parameter('apply_stiction_hysteresis', True)
        self.declare_parameter('apply_saturation', True)
        ...
        model_path = (
            Path(get_package_share_directory('wskr')) / 'WSKR' /
            'model_Model_R1_Hardware_heading.json'
        )
        if not model_path.exists():
            model_path = (
                Path(get_package_share_directory('wskr')) /
                'model_Model_R1_Hardware_heading.json'
            )

        self.model = JsonMLPModel(str(model_path))
        self.model_input = TemporalInputBuilder(self.model)

        # Paired params sidecar: captures training-time stiction, hysteresis,
        # and saturation. Resolved by swapping the 'model_' prefix with
        # 'params_' (same convention the trainer emits). Missing-file or
        # missing-keys degrade cleanly to passthrough.
        params_path = model_path.with_name(model_path.name.replace('model_', 'params_', 1))
        self.action_filter = ActionFilter(
            str(params_path) if params_path.exists() else None,
            apply_stiction_hysteresis=bool(self.get_parameter('apply_stiction_hysteresis').value),
            apply_saturation=bool(self.get_parameter('apply_saturation').value),
        )
        if self.action_filter.loaded_from:
            self.get_logger().info(
                f'Loaded action params sidecar: {self.action_filter.loaded_from} '
                f'(stiction_hysteresis={self.action_filter.apply_stiction_hysteresis}, '
                f'saturation={self.action_filter.apply_saturation})'
            )
        else:
            self.get_logger().warn(
                f'No action params sidecar found at {params_path} — '
                'stiction / hysteresis / saturation disabled (pure MLP output).'
            )
        ...
        self.control_timer = self.create_timer(
            1.0 / max(self.control_rate_hz, 1.0), self.control_loop, callback_group=cb_group
        )
    ...
    def whisker_callback(self, msg: Float32MultiArray) -> None:
        """Cache the latest 11-whisker drive-distance array for the control loop."""
    ...
    def _on_set_parameters(self, params) -> SetParametersResult:
        # Live-reconfigure lens params and action-filter toggles. Any other
        # parameter passes through with success.
        ...
        for p in params:
            if p.name == 'apply_stiction_hysteresis':
                self.action_filter.apply_stiction_hysteresis = bool(p.value)
                # Reset was_turning so the new mode starts cleanly.
                self.action_filter.reset()
                self.get_logger().info(
                    f'apply_stiction_hysteresis -> {self.action_filter.apply_stiction_hysteresis}'
                )
            elif p.name == 'apply_saturation':
                self.action_filter.apply_saturation = bool(p.value)
                self.get_logger().info(
                    f'apply_saturation -> {self.action_filter.apply_saturation}'
                )
    ...
    def control_loop(self) -> None:
        """Fixed-rate tick: feed whiskers + heading into the MLP, publish cmd_vel."""
        if self.active_goal_handle is None:
            return
        if self.latest_whiskers is None:
            return

        self.model_input.push_sensory(self.latest_whiskers / 1000.0, self.last_heading_deg)
        x = self.model_input.build_input()
        y = self.model.predict(x)

        # Apply training-time stiction / hysteresis / saturation so hardware
        # behavior matches the simulator the MLP was trained against.
        # action_hist is fed back to the next tick so the history reflects
        # what the robot ACTUALLY received, not the raw MLP output.
        cmd = Twist()
        if self.model.model_mode == 'heading':
            drive_mps = self.action_filter.clamp_drive_mps(float(y[0]))
            rot_dps = self.action_filter.process_rotation_dps(float(y[1]))
            cmd.linear.x = drive_mps
            cmd.angular.z = float(np.deg2rad(rot_dps))
            action_hist = np.array([drive_mps, rot_dps], dtype=np.float64)
        elif self.model.model_mode == 'xy_strafe':
            vx = self.action_filter.clamp_vx_mps(float(y[0]))
            vy = self.action_filter.clamp_vy_mps(float(y[1]))
            cmd.linear.x = vx
            cmd.linear.y = vy
            action_hist = np.array([vx, vy], dtype=np.float64)
        else:  # heading_strafe / unknown
            vx = self.action_filter.clamp_vx_mps(float(y[0]))
            vy = self.action_filter.clamp_vy_mps(float(y[1]) if y.shape[0] > 1 else 0.0)
            rot_dps_raw = float(y[2]) if y.shape[0] > 2 else 0.0
            rot_dps = self.action_filter.process_rotation_dps(rot_dps_raw)
            cmd.linear.x = vx
            cmd.linear.y = vy
            cmd.angular.z = float(np.deg2rad(rot_dps))
            action_hist = np.array([vx, vy, rot_dps], dtype=np.float64)

        self.cmd_pub.publish(cmd)
        self.model_input.push_action(action_hist)
    ...
    def execute_callback(self, goal_handle):
        ...
        with self.lock:
            ...
            # Fresh episode: drop any sensory/action history from a previous
            # approach so the policy sees copy-padded startup, not leftover
            # state from wherever the last run ended. Also reset the
            # rotation hysteresis one-bit state so was_turning starts False.
            self.model_input.reset()
            self.action_filter.reset()
        ...
        while rclpy.ok() and self.active_goal_handle is goal_handle:
            ...
            # Success when the tracked object itself reaches the whiskers
            # within proximity_success_mm. Requires a fresh valid track
            # (frames_since_valid_track == 0) so we don't flag success on
            # a stale bbox.
            if (
                self.last_tracked_bbox is not None
                and self.frames_since_valid_track == 0
                and self.latest_whiskers is not None
                and self._check_bbox_impinges_whiskers(
                    self.last_heading_deg,
                    self.latest_whiskers,
                    threshold_mm=self.proximity_success_mm,
                )
            ):
                result.proximity_success = True
                result.movement_message = (
                    f'Tracked object impinged on whiskers '
                    f'(within {self.proximity_success_mm:.0f} mm)'
                )
                goal_handle.succeed()
                break
